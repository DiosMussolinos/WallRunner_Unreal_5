...
void AWallRunCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind jump events
	PlayerInputComponent->BindAction("Jump", IE_Pressed, this, &AWallRunCharacter::Jump);
	PlayerInputComponent->BindAction("Jump", IE_Released, this, &AWallRunCharacter::StopJump);

	PlayerInputComponent->BindAction("AutoAim", IE_Pressed, this, &AWallRunCharacter::AutoAim);
	PlayerInputComponent->BindAction("ShowOthersPoints", IE_Pressed, this, &AWallRunCharacter::ShowOthers);
	PlayerInputComponent->BindAction("TurnOffWall", IE_Pressed, this, &AWallRunCharacter::Wall);
...
void AWallRunCharacter::OnFire()
{
	...
	FVector Foward = FirstPersonCameraComponent->GetForwardVector();

	//This is for a correction of angle. If the following line don't exist, an angle will exist and miss the shots.
	FVector End = Start + (Foward * 5000);
	...
	if (isHit)
	{
		if (Hit.GetActor())
		{
			...
			{
				ATarget* TargetCast = Cast<ATarget>(Hit.GetActor());

				if (!TargetCast->hit)
				{
					points += TargetCast->value;
					TargetCast->HitFunction(true);
				}
			}
		}
	}
	...
	if (!ACharacter::bPressedJump)
	{
		...
		if (wallWalk)
		{
			if (canWalk)
			{
				...
				#pragma region Dash 01 -> Downside -> One side only
...
void AWallRunCharacter::StopJump()
{
	if (ACharacter::bPressedJump)
	{
...
void AWallRunCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(AWallRunCharacter, points);
}

void AWallRunCharacter::AutoAim()
{
	if (autoTarget)
	{
		autoTarget = false;
